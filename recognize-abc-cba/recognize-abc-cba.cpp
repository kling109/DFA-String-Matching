// Identifier for the string "abc*cba"
#include <iostream>
#include <fstream>
#include <list>
...
list<int>* q4(ifstream &reader, int pos, list<int>* prevVals);
list<int>* q5(ifstream &reader, int pos, list<int>* prevVals);
...
list<int>* q3(ifstream &reader, int pos, list<int>* prevVals) {
    ...
    char newChar = reader.get();
    if (newChar == 'c') {
        return q4(reader, pos+1, prevVals);
    } else {
        return q3(reader, pos+1, prevVals);
    }
}
...
list<int>* q4(ifstream &reader, int pos, list<int>* prevVals) {
    ...
    char newChar = reader.get();
    if (newChar == 'b') {
        return q5(reader, pos+1, prevVals);
    } else if (newChar == 'c') {
        return q4(reader, pos+1, prevVals);
    } else {
        return q3(reader, pos+1, prevVals);
